...
// Binary wire protocol for DaiBed packets (see docs/P2P_IMPLEMENTATION.md).
// UDP and provider transports hand bytes to/from this serialization layer.
//
...
enum class MessageType : std::uint8_t
{
    ...
    PlayerCommand = 1,
    MatchSnapshot = 2,
    // Transport control messages (Phase 0.1R). Connect carries the password and
    // optional reconnect credentials; ConnectAck issues/rotates credentials.
    Connect = 3,
    ConnectAck = 4,
...
// v28: PlayerCommand carries a deduplicated UTF-8 chat message.
...
// Connect carries the server password and optional credentials from a previous
// accepted connection. ConnectAck returns a lobby client id plus credentials
// that must be presented to reclaim an in-match slot.
std::vector<std::uint8_t> EncodeConnect(std::uint32_t sequence, const ConnectRequest& request);
DecodeStatus DecodeConnect(
    const std::uint8_t* data, std::size_t size, PacketHeader& header, ConnectRequest& request);
std::vector<std::uint8_t> EncodeConnectAck(std::uint32_t sequence, const ConnectAccept& accept);
DecodeStatus DecodeConnectAck(
    const std::uint8_t* data, std::size_t size, PacketHeader& header, ConnectAccept& accept);
